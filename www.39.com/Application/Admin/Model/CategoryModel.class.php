...
class CategoryModel extends Model 
{
        ...
        public function floorData1()
	{
        ...
        public function getSearchConditionByGoodsId1($goodsId)
	{
		$ret = array();  // 返回的数组
		...
		->where(array(
            ...
            if($goodsCount > 1)
            {
                ...
                elseif($priceSection < 10000)
                    ...
                    $sectionCount = 7;
                //根据这些段数分段
                $pricePerSection = ceil($priceSection/$sectionCount);//每段范围
                $price = array();//存放最终的分段数据
                $firstPrice = 0;//第一个价格段的开始价格
                //循环每个段
                /*for($i=0;$i<$sectionCount;$i++)
			...
			$ret['price'] = $price;
                ...
                //处理这个属性数组：把属性相同的放到一起用属性名称作为下标->2维转3维
                $_gaData = array();
                foreach($gaData as $k => $v)
                {
                    $_gaData[$v['attr_name']][] = $v;
